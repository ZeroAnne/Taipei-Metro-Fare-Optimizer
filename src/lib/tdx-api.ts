 ...
 *
 * 接 API 步驟：
 *   1. 填入 CLIENT_ID / CLIENT_SECRET
 *   2. 呼叫 fetchStations() 取得車站清單，替換 stations.ts 的 STATIONS
 *   3. 呼叫 fetchAndCacheFares() 取得所有票價資料（建議存 localStorage 加速）
 *   4. 在 page.tsx 中改用 lookupFare() 替換 getMockFare()
 */

import type { TdxODFare } from './types'

// ── 填入申請到的憑證 ──────────────────────────────────────────────────────
const CLIENT_ID: string     = 'annastrid2000-1ebc9a28-11b3-42b8'  // TDX Client ID
const CLIENT_SECRET: string = '9538ddc3-2b5b-4e96-a204-4cfc534d4d28'  // TDX Client Secret
// ─────────────────────────────────────────────────────────────────────────
...
export async function fetchFares(): Promise<TdxODFare[]> {
  ...
  const res = await fetch(
    `${BASE_URL}/v2/Rail/Metro/ODFare/TRTC?${params}`,
    { headers: { Authorization: `Bearer ${token}` } }
  )
...
export async function fetchAndCacheFares(): Promise<TdxODFare[]> {
  ...
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({ data: fares, cachedAt: Date.now() }))
  } catch {
    // 超過 storage 限制時跳過
  }
